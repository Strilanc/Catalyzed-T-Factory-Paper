...
According to table 1 of \cite{ogorman2017factories}, assuming a physical gate error rate of $10^{-3}$ and a footprint of 17 million physical qubits, block factories can distill a CCZ state using on the order of $10^7$ qubit-rounds while achieving error rates on the order of $10^{-11}$.
Our factory's minimum footprint is much smaller, approximately 150 thousand qubits instead of millions, but our volume and error rates are 2-3 times larger.
...
Also, \cite{ogorman2017factories} does not produce an explicit physical layout.
In our experience, producing the physical layout often reveals unforeseen packing and routing overheads.)
Clearly block factories can outperform the efficiency of our factory, assuming enough states are being distilled in parallel.
...
We don't know the answer to this question, and leave it as future work.
